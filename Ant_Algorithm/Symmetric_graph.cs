...
internal class Symmetric_graph: IGraph_symmetry
{
                                      ...
                                      float Q)
    {
        for (var i = 0; i < (node.Key.Count - 1); i++) // перебор всех пунктов в маршруте
        {
            ...
            tempDictionary[node.Key[i]] = new KeyValuePair<float, float>(tempDictionary[node.Key[i]].Key, tempDictionary[node.Key[i]].Value + Q / node.Value);
            /////
        }
    }
}
